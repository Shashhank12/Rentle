...
<%
    ...
    if ((query != null && !query.trim().isEmpty()) || minPriceStr != null || maxPriceStr != null) {
        try {
            ...
            String sql = "SELECT items.item_id, items.name, items.description, items.location, items.posted_date, prices.price_per_hour, prices.price_per_day, prices.price_per_week, prices.price_per_month FROM items JOIN rentsfor ON (items.item_id = rentsfor.ItemID) JOIN prices ON (rentsfor.PricesID = prices.prices_id) WHERE (items.name LIKE ? OR items.description LIKE ?)";
            ...
            if (category != null && !category.equals("all")) {
                sql += " AND items.item_id IN (SELECT items.item_id FROM items JOIN has ON (items.item_id = has.ItemID) JOIN category ON (category.category_id = has.CategoryID) WHERE category_name LIKE ?)";
            }
            
            if (feature != null && !feature.equals("all")) {
                sql += " AND items.item_id IN (SELECT items.item_id FROM items JOIN has ON (items.item_id = has.ItemID) JOIN consistsof USING (CategoryID) JOIN features ON (consistsof.FeaturesID = features.features_id) WHERE features_name LIKE ?)";
            }
            
            if (sortBy.equals("price")) {
                sql += " ORDER BY prices.price_per_hour";
            ...
            while (rs.next()) {
                ...
                try{
                    itemCategory = rs.getString("category_name");
                } catch (Exception e) {
                    ps = con.prepareStatement("SELECT category_name FROM items JOIN has ON (items.item_id = has.ItemID) JOIN category ON (category.category_id = has.CategoryID) WHERE item_id = ? LIMIT 1");
                    ps.setInt(1, rs.getInt("item_id"));
                    ResultSet rs2 = ps.executeQuery();
                    if (rs2.next()) {
                        itemCategory = rs2.getString("category_name");
                    }
                }
                
                try {
                    itemFeature = rs.getString("features_name");
                } catch (Exception e) {
                    ps = con.prepareStatement("SELECT features_name FROM items JOIN has ON (items.item_id = has.ItemID) JOIN consistsof USING (CategoryID) JOIN features ON (consistsof.FeaturesID = features.features_id) WHERE item_id = ? LIMIT 1");
                    ps.setInt(1, rs.getInt("item_id"));
                    ResultSet rs2 = ps.executeQuery();
                    if (rs2.next()) {
                        itemFeature = rs2.getString("features_name");
                    }
                }
                
                try  {
                    String photoStatement = "SELECT photo FROM photos JOIN contains ON (photos.photo_id = contains.PhotoID) WHERE ItemID = ? LIMIT 1";
                    ps = con.prepareStatement(photoStatement);
                    ps.setInt(1, rs.getInt("item_id"));
                    ResultSet rs2 = ps.executeQuery();
                    if (rs2.next()) {
            ...
            %>
                <div class="grid_item">
                    <img src="<%=itemImage%>" onerror="this.src='images/profilepicgroup.png';" alt="" class="item_image">
