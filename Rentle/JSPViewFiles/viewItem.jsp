...
<%
    ...
    try {
        ...
        String query4 = "SELECT features_name FROM rentle.alsohave JOIN features USING (features_id) WHERE item_id = ?";
        ...
        String query5 = "SELECT user_id, first_name, last_name, profile_picture FROM user WHERE user_id = (SELECT UserID FROM rent WHERE ItemID = ?)";
        pstmt = con.prepareStatement(query5);
        pstmt.setString(1, currentItemId);
        rs = pstmt.executeQuery();

        if (rs.next()) {
            itemUserId = rs.getString(1);
            name = rs.getString(2) + " " + rs.getString(3);
            profilePic = rs.getString(4);
        }
        ...
        String query7 = "SELECT AVG(stars) from receives JOIN reviews ON (receives.ReviewID = reviews.review_id) GROUP BY UserID HAVING UserID = ?";
        pstmt = con.prepareStatement(query7);
        pstmt.setString(1, itemUserId);
        rs = pstmt.executeQuery();

        String reviewStars = "0.0";
        if (rs.next()) {
            reviewStars = rs.getString(1);
        }
        double reviewStarsDouble = Double.parseDouble(reviewStars);
        String formattedReviewStars = String.format("%.1f", reviewStarsDouble);

        String query8 = "SELECT COUNT(*) FROM receives WHERE UserID = ?";
        pstmt = con.prepareStatement(query8);
        pstmt.setString(1, itemUserId);
        rs = pstmt.executeQuery();

        String reviewCounts = "0";
        if (rs.next()) {
            reviewCounts = rs.getString(1);
        }
...
%>
        <div class="view_item_information_module">
            ...
            <div class="view_item_user_id" style="display:none"> <%=itemUserId%> </div>
            ...
            <div class="view_item_user_module">
                <img src="<%=profilePic%>" onerror="this.src='images/profilepicgroup.png';" class="view_item_user_profile_picture">
...
<%
        if (formattedReviewStars != "0.0" && reviewCounts != "0") {
%>
                    <div class="view_item_user_module_2">
                        <div class="view_item_reviews"> <%=formattedReviewStars%> </div>
                        <div class="view_item_reviews_count"> - <%=reviewCounts%> reviews </div>
                    </div>
<%      }%>
                </div>
            </div>
